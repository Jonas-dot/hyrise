// Scheduler-enabled naive DVI transaction benchmark.
// Identical workload to dvi_txn_benchmark_naive (no pooling, no compaction --
// dead MVCC rows accumulate, demonstrating O(N²) table-scan degradation) but
// with NodeQueueScheduler active throughout the run.
//
// Each operator pipeline is dispatched via OperatorTask::make_tasks_from_operator()
// + schedule_and_wait_for_tasks(), exactly as real Hyrise queries are executed.
// N OS threads provide concurrent transactions (MVCC axis); the scheduler's
// worker pool provides intra-operator chunk-level parallelism.
//
...
static const std::string TABLE_SINGLE = "dvi_naive_sched";
static const std::string TABLE_MULTI  = "dvi_naive_sched_multi";
...
static void sched_execute(const std::shared_ptr<hyrise::AbstractOperator>& root) {
  const auto& [tasks, _root_task] = hyrise::OperatorTask::make_tasks_from_operator(root);
...
static void setup_table(Mech m, int32_t pre_populate, int32_t num_unique_lhs,
  ...
  // Re-initialise scheduler after reset.
  // use_default_topology(N) caps workers at N cores; N=0 means all physical cores.
  hyrise::Hyrise::get().topology.use_default_topology(
      static_cast<uint32_t>(scheduler_workers));
  hyrise::Hyrise::get().set_scheduler(
      std::make_shared<hyrise::NodeQueueScheduler>());
...
struct Config {
  std::string experiment        = "NAIVE_SCHED";
  ...
  int         threads           = 1;
  int         scheduler_workers = 0;     // 0 = hardware_concurrency
  int         iterations        = 3;
...
static Config parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (k == "--scheduler-workers") cfg.scheduler_workers = std::stoi(next());
...
int main(int argc, char** argv) {
  ...
  const int effective_workers = (cfg.scheduler_workers > 0)
    ? cfg.scheduler_workers
    : static_cast<int>(std::thread::hardware_concurrency());

  std::cout << "dvi_txn_benchmark_naive_sched\n"
            ...
            << "  threads:           " << cfg.threads             << "  (concurrent transactions)\n"
            << "  scheduler_workers: " << effective_workers       << "  (intra-op parallelism)\n"
            << "  pre_pop:           " << pre_pop                 << "\n";
  ...
  for (int iter = 0; iter < cfg.iterations; ++iter) {
    std::cout << "\n=== Iteration " << (iter + 1) << " ===\n";
    setup_table(mech, pre_pop, num_unique_lhs, cfg.scheduler_workers);
      ...
      if (csv_out.is_open()) {
        if (!csv_header_written) {
          csv_out << "experiment,workload,mechanism,violation_rate,duplicate_rate,"
                     "access_pattern,size,threads,scheduler_workers,"
                     "iteration,batch_end,batch_ms,ops_per_s\n";
