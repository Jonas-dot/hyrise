#!/usr/bin/env bash
# =============================================================================
# run_naive.sh  –  Naive DVI transaction benchmark driver (T1-T6)
#
# Runs hyriseBenchmarkDVIndexNaive in the same experiment layout as run_txn.sh:
...
#
# Output:
#   results/naive_txn/<TIMESTAMP>/results.csv
#   results/naive_txn/<TIMESTAMP>/meta.txt
#   results/naive_txn/<TIMESTAMP>/run.log
#
# Usage:
#   ./scripts/txn/run_naive.sh --experiment T5 [--build] [--dry-run]
#   ./scripts/txn/run_naive.sh --experiment all [--binary PATH] [--outdir DIR]
#                              [--iterations N]
# =============================================================================
set -euo pipefail
...
# ---------------------------------------------------------------------------
BINARY="${HYRISE_ROOT}/build_release/hyriseBenchmarkDVIndexNaive"
EXPERIMENT=""
DRY_RUN=false
DO_BUILD=false
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
RESULT_BASE="${BENCH_ROOT}/results/naive_txn/${TIMESTAMP}"
...
if [[ "${DRY_RUN}" == "false" ]] && [[ ! -x "${BINARY}" ]]; then
  ...
  echo "       Build with: ./scripts/txn/run_naive.sh --build"
  echo "       Or manually: ninja -C ${HYRISE_ROOT}/build_release hyriseBenchmarkDVIndexNaive"
  exit 1
fi
...
# ---------------------------------------------------------------------------
# Experiment definitions (same T1-T6 structure as run_txn.sh)
# ---------------------------------------------------------------------------
run_T1() {
...
echo "======================================================================"
echo "  Naive DVI transaction benchmark (T1-T6)"
echo "  Binary      : ${BINARY}"
echo "  Experiment  : ${EXPERIMENT}"
echo "  Iterations  : ${ITERATIONS}"
...
if [[ "${DRY_RUN}" == "false" ]]; then
  mkdir -p "${BENCH_ROOT}/results/naive_txn"
  ln -sfn "${TIMESTAMP}" "${BENCH_ROOT}/results/naive_txn/latest"
fi
